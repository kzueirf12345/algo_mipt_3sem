...
#define ARITHMETIC_OVERLOADS(TYPE) \
    ...
    std::string to_hex() const {
        ...
        while (tmp) {
            auto [q, r] = tmp.divmod(16ul);
            uint64_t digit = static_cast<uint64_t>(r);
            s += hex_chars[digit];
            tmp = q;
        }
    ...
    uint64_t mod_short(uint64_t m) const {
        if (m == 0) {
            return 0;
        }
...
Integer power(Integer base, Integer exp, Integer mod) {
    Integer res{1ul};

    base %= mod;

    while (exp > Integer(0ul)) {
        if (exp % 2ul == Integer(1ul)) {
            res = res * base % mod;
        }
        base = base * base % mod;
        exp /= 2ul;
...
bool miller_rabin(Integer n) {
    if (n < Integer(2ul))            return false;
    if (n == Integer(2ul) || n == Integer(3ul)) return true;
    if (n % 2ul == Integer(0ul))       return false;

    Integer d = n - 1ul;
    int s = 0;
    while (d % 2ul == Integer(0ul)) {
        d /= 2ul;
        s++;
    }

    static const Integer bases[] = {2ul, 325ul, 9375ul, 28178ul, 450775ul, 9780504ul, 1795265022ul};
    ...
    for (Integer a : bases) {
        ...
        if (x == Integer(1ul) || x == n - 1ul) continue;
